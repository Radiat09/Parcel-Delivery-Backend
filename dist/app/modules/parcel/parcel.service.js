...
const createParcelService = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (isUserExist.role === user_interface_1.Role.DELIVERY_MAN ||
    ...
    if (user.role === user_interface_1.Role.ADMIN || user.role === user_interface_1.Role.SUPER_ADMIN) {
        ...
        const [data, meta] = yield Promise.all([
            users
                ...
                .select("-sender.id -sender._id -_id"),
...
const updateParcelService = (req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Check authorization
    if (user.role === user_interface_1.Role.SENDER && !parcel.sender.equals(user.userId)) {
    ...
    switch (user.role) {
        ...
        case user_interface_1.Role.SENDER: {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const updates = {};
            if (updateData.receiver && parcel.currentStatus === parcel_interface_1.EStatus.REQUESTED) {
                Object.entries(updateData.receiver).forEach(([key, value]) => {
                    updates[`receiver.${key}`] = value;
                });
            }
        ...
        case user_interface_1.Role.DELIVERY_MAN: {
            ...
            if (updateData.currentStatus &&
                ...
                throw new Error(`You can not update this status ${Object.values(updateData)}`);
            }
            if (updateData.currentStatus) {
    ...
    // Perform the update
    const updatedParcel = yield parcel_model_1.Parcel.findOneAndUpdate({ trackingId: trkId }, update, { new: true, runValidators: true })
        .populate("sender", "name email phone")
